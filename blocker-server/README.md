...
```bash
blocker-server$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
blocker-server$ sam local start-api
blocker-server$ curl http://localhost:3000/classify_digit
```
...
```bash
blocker-server$ sam logs -n InferenceFunction --stack-name blocker-server --tail
```
...
```bash
aws cloudformation delete-stack --stack-name blocker-server
```
